...
import { ParticleCanvas, WeatherIconAnim, AnimatedTemp, StatBar } from "./components/WeatherComponents";
...
export default function App() {
  ...
  return (
    <div
      className="page"
      style={{ background: `linear-gradient(160deg, ${bg[0]} 0%, ${bg[1]} 50%, ${bg[2]} 100%)` }}
    >
      ...
      <div ref={mainRef} className="main-wrapper">
        ...
        <form onSubmit={handleSearch} className="search-form">
          ...
          <button type="submit" className="search-button" style={{ background: accent }}>
            Rechercher
          </button>
        </form>

        {error && <div ref={errorRef} className="error-box">⚠️ {error}</div>}
        ...
        {weather && !loading && (
          <div className="weather-grid">
            ...
            <div className="left-col">
                  ...
                  <div ref={titleRef}>
                    ...
                    <div className="temp-wrapper">
                      ...
                      <p className="feels-like">
                        Ressenti {Math.round(weather.current.feelslike_c)}° • Min{" "}
                        {Math.round(weather.forecast.forecastday[0].day.mintemp_c)}° / Max{" "}
              ...
              <div className="panel">
                <p className="stat-label">⏰ Aujourd'hui — heure par heure</p>
                <div className="hourly-scroll">
                  <div
                    className="hourly-grid"
                    style={{
                      gridTemplateColumns: `repeat(${hourly.length}, minmax(72px, 1fr))`,
                    }}
                  >
                    {hourly.map((h, i) => (
                      <div key={h.time} ref={(el) => (hourlyRefs.current[i] = el)} className="hourly-card">
                        <p className="hourly-time">
                          {new Date(h.time).toLocaleTimeString("fr-FR", { hour: "2-digit", minute: "2-digit" })}
                        </p>
                        <div className="hourly-icon">{getWeatherIcon(h.condition.code)}</div>
              ...
              <div className="panel panel-info-grid">
                {getExtraInfo(weather.current, weather.forecast.forecastday[0]).map((info, i) => (
                  <div key={info.label} ref={(el) => (statsRefs.current[4 + i] = el)}>
                    <p className="stat-label">{info.label}</p>
                    <p className="stat-value" style={{ fontSize: "1.1rem" }}>{info.value}</p>
                  </div>
                ))}
              </div>
            ...
            <div className="panel panel-right">
              <p className="stat-label forecast-section-label">📅 Prévisions 5 jours</p>
                ...
                return (
                  ...
                  >
                    <div className="forecast-day">
                      <p className="forecast-day-name" style={{ fontWeight: isToday ? 700 : 500, color: isToday ? "white" : "rgba(255,255,255,0.75)" }}>
                        {isToday
                          ? "Aujourd'hui"
                    ...
                    <span className="forecast-icon">{getWeatherIcon(day.day.condition.code)}</span>
                    <div className="forecast-temps">
                      <p className="forecast-max">{Math.round(day.day.maxtemp_c)}°</p>
              ...
              <div className="mini-cards-grid">
                {getMiniCards(weather.current, weather.location.localtime).map((info) => (
                  <div key={info.label} className="mini-card">
                    <p className="stat-label">{info.icon} {info.label}</p>
